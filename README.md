...
- 成长 V1 发布前已在设备 `7c2b63fd4a128` 创建并校验完整恢复点
  `/root/ai-cat-backups/before-growth-v1-20260811T093252Z-3748`，回滚脚本
  `--check`、SQLite 完整性和 SHA-256 清单均通过。板端已部署开关保护版本，但
  `AI_CAT_ENABLE_GROWTH_PERSONALITY_V1=false` 保持关闭。
- 实体头部触摸已通过现场验收：一次连续触摸产生的 4 条厂商原始记录经防抖后只
  导入 1 个业务事件，并正确点头和播放“摸摸头，好舒服呀。”。鼻部 `GPIO113`
  经厂商日志和独立 60 秒 GPIO 边沿监听均无输入，暂记为硬件传感器/排线待检；
  背部和左右爪按现场决定暂时跳过，不能视为已通过验收。
- 自动化测试更新为 `216 passed`，覆盖成长总开关关闭后亲密度、对话和基础人格
  不受影响，数据库不产生新的成长事件，且以后重新开启时不会追溯计算关闭期间已
  导入的真机对话。
