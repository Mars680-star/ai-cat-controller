...
- 5 种性格各增加 5 条对应音色的本地触摸短语，共 25 个 WAV：动作完成后按头部、
  背部、鼻部、左脚和右脚播放固定短句。播放不启动火山云端，使用本地语音标记
...
- 当前持久化性格会在空闲阶段通过 `session.update` 同步真实提示词、音色和动作
  规则；已实现电量查询、天气及头部/尾部动作 Function Calling。
...
- 触摸动作完成后按“性格 + 部位”播放本地固定语句；5 种性格共 25 个触摸 WAV，
