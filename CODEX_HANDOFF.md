...
- `7c2b63fd4a128` 的成长 V1 发布恢复点为
  `/root/ai-cat-backups/before-growth-v1-20260811T093252Z-3748`，校验已通过；板端
  已部署总开关和 1.5 秒触摸防抖，但成长总开关仍为 `false`。头部触摸已通过；
  鼻部 `GPIO113` 在厂商日志和独立 GPIO 监听中均无信号，属于硬件待检；背部和
  左右爪由用户暂时跳过，不能标记为已验收。
...
1. 检修 `7c2b63fd4a128` 鼻部传感器或排线，使 `GPIO113` 能产生边沿；随后补做
   鼻部、背部和左右爪验收，再决定是否开启成长人格 V1 总开关。
...
7. 控制台更换音色后重新采集并抽听 20 个本地 WAV；完成 RTC、视频和厂商 SDK
   其余未覆盖 API 的测试。
8. 在用户安排真机阶段，验证一次完整真实问答生成成长事件、标签变化刷新 runtime，
   并由下一次 `session.update` 体现稳定行为差异；当前仅完成 Mock 与软件回归。
