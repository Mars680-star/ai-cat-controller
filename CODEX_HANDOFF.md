...
- 5 个亲密度等级、每日增长上限、称呼/动作解锁和 SQLite 持久化。
...
- 头部、背部、左右脚和鼻部实体触摸从 `toy_main` 日志增量导入亲密度，复用
  request ID 幂等、每日次数和总增长上限；完成任务当前由网页按钮确认。
...
- 旧 DDS 电机执行入口已替换；触摸会增加亲密度，但当前不会触发电机。
...
2. 在统一安全调度器内设计触摸动作反馈，保持亲密度导入与电机执行解耦。
...
7. 逐一人工验收其余性格音色；完成 RTC、视频和厂商 SDK 其余未覆盖 API 的测试。
