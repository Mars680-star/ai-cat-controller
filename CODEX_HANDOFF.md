...
- 亲密度跨级可由 `AI_CAT_ENABLE_LEVEL_UP_CELEBRATION=true` 启用一次性升级庆祝：
  等待对话和动作空闲后执行固定 `celebration_combo`，完成后播放共享
  `assets/local-speech/shared/event-level_up.wav`。重复互动不重复触发，触摸升级时
  庆祝优先；本地提示语不启动火山服务，并固定走实体 PulseAudio 输出。采集脚本
  必须等待启动欢迎语结束，避免把“你好主人，我是小安”误存为业务语音。
...
7. 控制台更换音色后重新采集并抽听 21 个本地 WAV；完成 RTC、视频和厂商 SDK
   其余未覆盖 API 的测试。
