...
- `adapters/`: in-memory Mock behavior and the fixed-command Local K1 boundary.
...
7. Stop active motion during application shutdown.
...
motion is compiled into the native program but is omitted from adapter
capabilities unless `AI_CAT_ENABLE_TAIL_MOTION=true`; the default API result is
`501`.
