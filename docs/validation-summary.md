...
原厂 `toy_motor.service` 的 `/usr/bin/toy_control` 执行路径已停用。日志确认其
上游 `toy_main.service` 会周期性发布 DDS 自主动作，直接恢复厂商执行器会绕过
电机锁并造成双控制。2026-08-03 起，同名服务改为运行本项目安全自主行为进程，
只通过 FastAPI 调用已验收的头部动作；故障尾部不会被随机动作选中。
